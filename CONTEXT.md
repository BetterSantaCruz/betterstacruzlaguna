...
- Current local QA is green for civic-data validation, 391 unit tests, lint, TypeScript, Prettier, the production build, and the 11-test Santa Cruz browser smoke boundary. Re-run these gates after each feature slice; the broader inherited E2E files remain retained architecture/reference coverage.
...
- Source snapshots are retained selectively and immutably. Do not write into the Fensalir vault's `raw/` archive.
...
Preserve the BetterLB/OpenLGU separation:

- source intake and raw-preserving collection observe upstream rows;
- staging normalizes untrusted candidates;
- validation and reconciliation produce evidence and review items;
- only an explicit promotion step may update canonical records;
...
The architecture is intentionally retained while gated modules remain empty. The published Santa Cruz baseline is limited to directly inspectable PSA/DBM records; schemas, provenance checks, canonical cross-file validation, source-ledger UI, and explicit data-gap states remain the safe v1 foundation for everything else.
