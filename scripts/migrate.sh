...
# BetterSantaCruz - Cloudflare D1 Migration Management
#
# Usage:
...
DB_BINDING="BETTERSANTACRUZ_DB"
DB_NAME="${BETTERSANTACRUZ_DB_NAME:-}"
...
require_remote_database() {
    if [ -z "$DB_NAME" ]; then
        log_error "Remote D1 is disabled until BETTERSANTACRUZ_DB_NAME is explicitly configured."
        exit 1
    fi
}
