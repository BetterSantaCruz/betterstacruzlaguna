...
const DEFAULT_OUTPUT_ROOT = null;
...
function parseArgs(argv) {
  const args = {
    source: null,
    outputRoot:
      process.env.BETTERSANTACRUZ_SOURCE_OUTPUT_ROOT || DEFAULT_OUTPUT_ROOT,
  };
...
Options:
  --source <key>          Diff a single configured source only
  --output-root <path>    Reviewed source output root (required)
`);
}
...
function main() {
  ...
  if (!args.outputRoot) {
    throw new Error(
      'Source diffing is disabled until a reviewed source output root is provided with --output-root or BETTERSANTACRUZ_SOURCE_OUTPUT_ROOT.'
    );
  }
  const keys = args.source
    ? [args.source]
    : fs
        .readdirSync(args.outputRoot, { withFileTypes: true })
