...
const PIPELINE_DIR = process.env.BETTERSANTACRUZ_PIPELINE_DIR
  ? path.resolve(ROOT, process.env.BETTERSANTACRUZ_PIPELINE_DIR)
  : null;
const OFFICIAL_SOURCE_BASE_URL = String(
  process.env.BETTERSANTACRUZ_OFFICIAL_SOURCE_BASE_URL || ''
).replace(/\/+$/, '');

const PATHS = PIPELINE_DIR
  ? {
      stagedDocuments: path.join(PIPELINE_DIR, 'staged-documents.jsonl'),
      stagedPersonRefs: path.join(PIPELINE_DIR, 'staged-person-refs.jsonl'),
      sourceRecords: path.join(PIPELINE_DIR, 'source-records.jsonl'),
      reconciliationShadow: path.join(
        PIPELINE_DIR,
        'reconciliation-shadow.json'
      ),
      decisions: path.join(PIPELINE_DIR, 'review-decisions.jsonl'),
      terms: path.join(PIPELINE_DIR, 'terms.json'),
      fallbackTerms: path.join(PIPELINE_DIR, 'reference/terms.json'),
      sourcesRoot: path.join(PIPELINE_DIR, 'sources'),
    }
  : {};
...
function writeJsonlAppend(filePath, record) {
  if (!filePath) {
    throw new Error(
      'Review workbench writes are disabled until BETTERSANTACRUZ_PIPELINE_DIR is configured.'
    );
  }
...
function loadSourceRuns() {
  if (!PATHS.sourcesRoot || !fs.existsSync(PATHS.sourcesRoot)) return [];
  return fs
...
function loadSnapshot() {
  if (!PIPELINE_DIR) {
    return {
      ok: false,
      error:
        'No pipeline artifact directory configured. Set BETTERSANTACRUZ_PIPELINE_DIR for a reviewed local run.',
...
function officialPdfUrl(pdfUrl) {
  ...
  if (!OFFICIAL_SOURCE_BASE_URL) return '';
  return `${OFFICIAL_SOURCE_BASE_URL}/${String(pdfUrl).replace(/^\/+/, '')}`;
}
