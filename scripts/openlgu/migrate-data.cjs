...
function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`Usage: BETTERSANTACRUZ_ENABLE_LEGACY_MIGRATION=true node scripts/openlgu/migrate-data.cjs [output.sql]

Legacy upstream migration helper. It is disabled by default and must only be
used after a reviewed Santa Cruz-specific migration input has been prepared.
...
`);
  ...
  if (process.env.BETTERSANTACRUZ_ENABLE_LEGACY_MIGRATION !== 'true') {
    throw new Error(
      'Legacy migration is disabled. Do not run inherited migration inputs against Santa Cruz; prepare and review a municipality-specific migration first.'
    );
  }
