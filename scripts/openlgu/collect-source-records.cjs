...
function parseArgs(argv) {
  const args = {
    input: process.env.BETTERSANTACRUZ_MANUAL_SOURCE_INPUT || DEFAULT_INPUT,
...
Options:
  --input <path>    Reviewed/manual staging CSV (required)
...
function main() {
  ...
  if (!args.input) {
    throw new Error(
      'Manual source collection is disabled until a reviewed CSV is provided with --input or BETTERSANTACRUZ_MANUAL_SOURCE_INPUT.'
    );
  }
