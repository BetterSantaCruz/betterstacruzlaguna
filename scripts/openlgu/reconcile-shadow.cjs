...
const DEFAULT_STAGED_DOCUMENTS = null;
...
function parseArgs(argv) {
  const args = {
    stagedDocuments:
      process.env.BETTERSANTACRUZ_STAGED_DOCUMENTS || DEFAULT_STAGED_DOCUMENTS,
    out: DEFAULT_OUT,
...
Options:
  --staged-documents <path>    Reviewed staged document JSONL (required)
...
function main() {
  ...
  if (!args.stagedDocuments) {
    throw new Error(
      'Shadow reconciliation is disabled until a reviewed staged document artifact is provided with --staged-documents or BETTERSANTACRUZ_STAGED_DOCUMENTS.'
    );
  }
