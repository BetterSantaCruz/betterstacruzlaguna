...
const DB_BINDING =
  process.env.BETTERSANTACRUZ_DB_BINDING || 'BETTERSANTACRUZ_DB';
...
function wranglerCommand(sql, isRemote) {
  ...
  try {
    ...
    const cmd = `${npx} wrangler d1 execute ${DB_BINDING} ${flag} --file="${tempFile}"`;
...
function loadSourceRecords(isRemote) {
  ...
  const sourceRecords = readJsonl(
    path.join(pipelineDir, 'source-records.jsonl')
  );
  ...
  if (sourceKeys.length) {
    const scrapeSourcesSql = sourceKeys
      .map(key => {
        ...
        const source = sourceRecords.find(record => record.source_key === key);
        let baseUrl = null;
        try {
          baseUrl = source?.source_url
            ? new URL(source.source_url).origin
            : null;
        } catch {
          baseUrl = null;
        }
        return `INSERT OR IGNORE INTO scrape_sources (id, name, source_type, base_url) VALUES (${escapeSql(id)}, ${escapeSql(key)}, 'lgu_website', ${escapeSql(baseUrl)});`;
      })
      .join('\n');
...
function loadStagedDocuments(isRemote) {
  ...
  const stagedDocs = readJsonl(
    path.join(pipelineDir, 'staged-documents.jsonl')
  );
...
function loadStagedPersonRefs(isRemote) {
  ...
  const personRefs = readJsonl(
    path.join(pipelineDir, 'staged-person-refs.jsonl')
  );
...
function loadTerms(isRemote) {
  console.log('\nLoading terms...');
  const termsPath = path.join(pipelineDir, 'terms.json');
...
function main() {
  ...
  const pipelineFlagIndex = args.indexOf('--pipeline-dir');
  const configuredPipelineDir =
    pipelineFlagIndex >= 0 ? args[pipelineFlagIndex + 1] : null;
  const envPipelineDir = process.env.BETTERSANTACRUZ_PIPELINE_DIR || null;
  const selectedPipelineDir = configuredPipelineDir || envPipelineDir;

  if (!selectedPipelineDir) {
    console.error(
      '\nError: pipeline loading is disabled until a reviewed artifact directory is explicitly provided with --pipeline-dir or BETTERSANTACRUZ_PIPELINE_DIR.'
    );
    process.exit(1);
  }

  pipelineDir = path.resolve(ROOT, selectedPipelineDir);
  ...
  if (!fs.existsSync(pipelineDir)) {
    console.error(`\nError: Pipeline directory not found: ${pipelineDir}`);
    process.exit(1);
  }
