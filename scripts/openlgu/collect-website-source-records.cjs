...
const COLLECTOR_VERSION = 'lgu-website-collector-v1';
const DEFAULT_OUTPUT_ROOT = 'pipeline/openlgu/sources';
const SOURCE_CONFIG_ENV = 'BETTERSANTACRUZ_SOURCE_CONFIG';

function loadSources(configPath) {
  if (!configPath) {
    throw new Error(
      `No source configuration provided. Set ${SOURCE_CONFIG_ENV} or pass --source-config with reviewed source definitions.`
    );
  }

  const resolvedPath = path.resolve(configPath);
  if (!fs.existsSync(resolvedPath)) {
    throw new Error(`Source configuration not found: ${resolvedPath}`);
  }
  ...
  if (!Array.isArray(entries) || entries.length === 0) {
    throw new Error(
      'Source configuration must contain a non-empty sources array.'
    );
  }

  return entries.map((source, index) => {
    const required = ['key', 'label', 'url', 'table_selector', 'document_type'];
    const missing = required.filter(field => !source[field]);
    if (missing.length > 0) {
      throw new Error(
        `Source definition ${index + 1} is missing: ${missing.join(', ')}`
      );
    }
    if (!/^[a-z0-9_-]+$/i.test(source.key)) {
      throw new Error(`Invalid source key: ${source.key}`);
    }
    try {
    ...
    return {
      ...source,
      fallback_selectors: Array.isArray(source.fallback_selectors)
        ? source.fallback_selectors
        : [],
      column_mappings: source.column_mappings.map(mapping => {
        if (!mapping.header_pattern || !mapping.field) {
          throw new Error(
            `Source ${source.key} contains an incomplete column mapping.`
          );
        }
        return {
          ...mapping,
          header_pattern: new RegExp(String(mapping.header_pattern), 'i'),
        };
      }),
    };
  });
...
function parseArgs(argv) {
  const args = {
    source: null,
    sourceConfig: process.env[SOURCE_CONFIG_ENV] || null,
    htmlDir: null,
    saveHtmlDir: null,
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (arg === '--source' && next) {
      args.source = next;
      i += 1;
    } else if (arg === '--source-config' && next) {
...
Fetches reviewed LGU legislative table definitions, parses rows, and emits source-record JSONL.
No website is contacted until a reviewed source configuration is supplied.

Options:
  --source-config <path>  Reviewed JSON source definitions (or ${SOURCE_CONFIG_ENV})
  --source <key>          Collect one configured source only
...
function writeCombinedOutput(outputRoot, sources) {
  const combined = [];
...
async function main() {
  ...
  const configuredSources = loadSources(args.sourceConfig);
  const sources = args.source
    ? configuredSources.filter(s => s.key === args.source)
    : configuredSources;
  ...
  writeCombinedOutput(args.outputRoot, sources);
}
