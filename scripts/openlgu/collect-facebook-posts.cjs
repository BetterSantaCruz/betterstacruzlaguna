...
const DEFAULT_SOURCE_KEY = 'lgu_facebook_sessions';
const DEFAULT_INPUT_DIR = null;
...
function parseArgs(argv) {
  const args = {
    inputDir:
      process.env.BETTERSANTACRUZ_FACEBOOK_INPUT_DIR || DEFAULT_INPUT_DIR,
    outputRoot: DEFAULT_OUTPUT_ROOT,
    sourceKey:
      process.env.BETTERSANTACRUZ_FACEBOOK_SOURCE_KEY || DEFAULT_SOURCE_KEY,
  };
...
Options:
  --input-dir <path>     Reviewed, normalized Facebook fixture directory (required)
  --output-root <path>   Pipeline sources directory. Default: ${DEFAULT_OUTPUT_ROOT}
  --source-key <key>     Reviewed source key. Default: ${DEFAULT_SOURCE_KEY}
`);
}
...
function toSourceRecord(rawPayload, runId, sourceKey) {
...
function main() {
  ...
  if (!/^[a-z0-9_-]+$/i.test(args.sourceKey)) {
    throw new Error(`Invalid source key: ${args.sourceKey}`);
  }
  if (!args.inputDir) {
    throw new Error(
      'Facebook collection is disabled until a reviewed fixture directory is provided with --input-dir or BETTERSANTACRUZ_FACEBOOK_INPUT_DIR.'
    );
  }
  ...
  for (const f of files) {
    ...
    for (const doc of docs) {
      allRecords.push(toSourceRecord(doc, runId, args.sourceKey));
    }
  }
  ...
  writeRunOutput(args.sourceKey, manifest, allRecords, args.outputRoot);
