...
const DEFAULT_TERMS = null;
const DEFAULT_ROSTER = null;
...
function parseArgs(argv) {
  const args = {
    input: process.env.BETTERSANTACRUZ_SOURCE_RECORDS_INPUT || DEFAULT_INPUT,
    terms: process.env.BETTERSANTACRUZ_TERMS_INPUT || DEFAULT_TERMS,
    roster: process.env.BETTERSANTACRUZ_TERM_ROSTER_INPUT || DEFAULT_ROSTER,
...
Options:
  --input <path>                 Reviewed source records JSONL (required)
  --terms <path>                 Reviewed terms JSON (required)
  --roster <path>                Reviewed term roster JSON (optional)
...
function main() {
  ...
  if (!args.input || !args.terms) {
    throw new Error(
      'Document staging is disabled until reviewed source records and term JSON are provided with --input/--terms or their BETTERSANTACRUZ_* environment variables.'
    );
  }
