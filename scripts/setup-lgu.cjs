...
/**
 * setup-lgu.cjs
 * Interactive CLI wizard to configure a new civic portal fork.
 *
...
async function main() {
  ...
  // ─── Collect info ──────────────────────────────────────────────────
  const lguName = await question(rl, 'LGU name (e.g. "Sample Municipality")');
  ...
  const enableStats =
    (
      await question(rl, 'Enable statistics module? (y/n)', 'n')
    ).toLowerCase() === 'y';
  const enableOpenLGU =
    (await question(rl, 'Enable OpenLGU portal? (y/n)', 'n')).toLowerCase() ===
    'y';
  const enableTransparency =
    (
      await question(rl, 'Enable transparency portal? (y/n)', 'n')
    ).toLowerCase() === 'y';
  ...
  const config = {
    ...
    location: {
      coordinates: null,
      weather: { enabled: false, defaultCity: '' },
    },
    transparency: {
  ...
  try {
    ...
    const slug = slugify(lguName);
    const replacements = {
      '/logos/svg/betterexample-logo-primary.svg': `/logos/svg/${slug}-logo-primary.svg`,
      '/logos/png/betterexample-banner-inverted.png': `/logos/png/${slug}-banner-inverted.png`,
      '/logos/png/betterexample-blue.png': `/logos/png/${slug}-blue.png`,
    };
