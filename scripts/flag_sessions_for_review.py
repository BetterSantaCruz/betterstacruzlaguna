...
DB_NAME = "BETTERSANTACRUZ_DB"
LOCAL_DB_PATH = ".wrangler/state/v3/d1/miniflare-D1DatabaseObject"
REMOTE_DB_NAME = os.environ.get("BETTERSANTACRUZ_REMOTE_DB_NAME", "").strip()


def get_local_db_path() -> str:
    """Find the local D1 database file."""
    configured_path = os.environ.get("BETTERSANTACRUZ_LOCAL_DB_PATH", "").strip()
    if configured_path:
        full_path = os.path.abspath(configured_path)
        if os.path.isfile(full_path):
            return full_path
        raise FileNotFoundError(
            f"Configured BETTERSANTACRUZ_LOCAL_DB_PATH does not exist: {full_path}"
        )
    ...
    candidates = []
    if os.path.isdir(wrangler_dir):
        for root, _dirs, files in os.walk(wrangler_dir):
            candidates.extend(
    ...
    if len(candidates) > 1:
        raise FileNotFoundError(
            "Multiple local D1 databases found. Set BETTERSANTACRUZ_LOCAL_DB_PATH "
            "to the reviewed database file."
        )
...
def require_remote_db_name() -> str:
    """Require an explicitly configured remote database name."""
    if not REMOTE_DB_NAME:
        raise RuntimeError(
            "Remote D1 access is disabled until BETTERSANTACRUZ_REMOTE_DB_NAME "
...
def execute_query(query: str, params: Tuple = (), use_remote: bool = False) -> List[Dict[str, Any]]:
    ...
    if use_remote:
        remote_db_name = require_remote_db_name()
