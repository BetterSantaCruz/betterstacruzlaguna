...
const CriticalHotlinesWidget: FC<CriticalHotlinesWidgetProps> = ({
  ...
  if (displayedHotlines.length === 0) {
    return (
      <DataStatus
        title='Emergency hotline data is not published yet'
        message='No Santa Cruz-specific hotline record has passed the source and freshness checks. Use official emergency channels directly for urgent situations.'
        sourceHref='/sources'
      />
    );
  }
  ...
  return (
    <div className='overflow-hidden rounded-lg border shadow-md border-kapwa-border-weak bg-kapwa-bg-surface'>
      <div className='flex justify-between items-center px-4 py-3 bg-kapwa-bg-danger-default'>
        ...
        >
          View source status <ChevronRightIcon className='ml-1 w-4 h-4' />
        </Link>
      </div>
