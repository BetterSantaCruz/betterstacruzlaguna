...
async function handleGetDuplicates(context: {
  ...
  try {
    ...
    const exactResults = await env.BETTERSANTACRUZ_DB.prepare(sql_exact).all();
    ...
    for (const row of exactResults.results as Array<{
      ...
      const personRecordsResult = await env.BETTERSANTACRUZ_DB.prepare(
      ...
      const docCount = await env.BETTERSANTACRUZ_DB.prepare(
        `SELECT COUNT(*) as count FROM document_authors WHERE person_id IN (${ids.map(() => '?').join(',')})`
      )
        .bind(...ids)
        .first<{ count: number }>();

      const memberCount = await env.BETTERSANTACRUZ_DB.prepare(
        `SELECT COUNT(*) as count FROM memberships WHERE person_id IN (${ids.map(() => '?').join(',')})`
      )
        .bind(...ids)
        .first<{ count: number }>();

      const committeeCount = await env.BETTERSANTACRUZ_DB.prepare(
    ...
    for (const row of middleResults.results as Array<{
      ...
      const personsResult = await env.BETTERSANTACRUZ_DB.prepare(
...
async function handleMerge(context: {
  ...
  try {
    ...
    const keepPerson = await env.BETTERSANTACRUZ_DB.prepare(
    ...
    for (const id of merge_person_ids) {
      const person = await env.BETTERSANTACRUZ_DB.prepare(
        `SELECT id FROM persons WHERE id = ?1`
      )
        .bind(id)
        .first();
    ...
    const results = await env.BETTERSANTACRUZ_DB.batch(statements);
