...
async function handleGetSession(context: {
  ...
  try {
    ...
    const session = await env.BETTERSANTACRUZ_DB.prepare(
    ...
    const membersResult = await env.BETTERSANTACRUZ_DB.prepare(
      `SELECT
        m.id,
        m.person_id,
        p.first_name,
...
async function handleUpdateSession(context: {
  ...
  try {
    ...
    const updated = await env.BETTERSANTACRUZ_DB.prepare(
