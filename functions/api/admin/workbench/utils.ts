...
export function officialPdfUrl(
  pdfUrl: string,
  officialSourceBaseUrl = ''
): string {
  if (!pdfUrl) return '';
  if (/^https?:\/\//i.test(pdfUrl)) return pdfUrl;
  if (!officialSourceBaseUrl) return '';

  try {
    const base = new URL(officialSourceBaseUrl);
    if (!['http:', 'https:'].includes(base.protocol)) return '';
    return new URL(pdfUrl.replace(/^\/+/, ''), `${base.origin}/`).toString();
  } catch {
    return '';
  }
}
