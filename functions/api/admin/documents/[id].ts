...
async function handlePatchDocument(context: {
  ...
  try {
    ...
    if (updateFields.length > 0) {
      ...
      await env.BETTERSANTACRUZ_DB.prepare(updateSql)
        .bind(...updateValues)
        .run();
    }
    ...
    if (body.authors !== undefined) {
      ...
      for (const author of body.authors) {
        ...
        if (!personId || personId.startsWith('temp_')) {
          ...
          const existingPerson = await env.BETTERSANTACRUZ_DB.prepare(
    ...
    if (body.subjects !== undefined) {
      ...
      for (const subjectName of body.subjects) {
        ...
        let subject = await env.BETTERSANTACRUZ_DB.prepare(
    ...
    const doc = await env.BETTERSANTACRUZ_DB.prepare(
...
async function handleGetDocument(context: {
  ...
  try {
    ...
    const doc = await env.BETTERSANTACRUZ_DB.prepare(sql)
      .bind(documentId)
      .first<any>();
    ...
    `;
    const subjectsResult = await env.BETTERSANTACRUZ_DB.prepare(subjectsSql)
      .bind(documentId)
      .all();
