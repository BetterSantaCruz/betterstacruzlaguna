...
async function handleGetStats(context: {
  ...
  try {
    ...
    const reviewQueueStats = await env.BETTERSANTACRUZ_DB.prepare(
      `SELECT
        COUNT(*) as total,
    ...
    // Get document stats
    const documentStats = await env.BETTERSANTACRUZ_DB.prepare(
      `SELECT
        COUNT(*) as total,
    ...
    try {
      const errorStats = await env.BETTERSANTACRUZ_DB.prepare(
        `SELECT
          COUNT(*) as total,
    ...
    try {
      const conflictStats = await env.BETTERSANTACRUZ_DB.prepare(
    ...
    try {
      const deletionQueueStats = await env.BETTERSANTACRUZ_DB.prepare(
