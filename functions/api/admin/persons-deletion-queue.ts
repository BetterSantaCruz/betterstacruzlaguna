...
async function handleGetQueue(context: {
  ...
  try {
    ...
    const results = await env.BETTERSANTACRUZ_DB.prepare(sql).bind(limit).all();
...
async function handlePermanentDelete(context: {
  ...
  try {
    ...
    const authorCount = await env.BETTERSANTACRUZ_DB.prepare(
    ...
    await env.BETTERSANTACRUZ_DB.prepare(`DELETE FROM persons WHERE id = ?1`)
      .bind(person_id)
      .run();
