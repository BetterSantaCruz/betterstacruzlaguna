    ...
    const result = await kvCache.get(
      ...
      async () => {
        ...
        const memberCountsSql = `
          ...
          GROUP BY term_id
        `;
        const memberCountsResult = await env.BETTERSANTACRUZ_DB.prepare(
          memberCountsSql
        )
          .bind(...termIds)
          .all<MemberCountRow>();
    ...
    const result = await kvCache.get(
      ...
      async () => {
        const termSql = `
          ...
          WHERE t.id = ?
        `;
        const term = await env.BETTERSANTACRUZ_DB.prepare(termSql)
          .bind(termId)
          .first<TermDetailRow>();
        ...
        `;
        const membersResult = await env.BETTERSANTACRUZ_DB.prepare(membersSql)
          .bind(termId)
          .all();
        ...
        const statsSql = `
          ...
          WHERE term_id = ?
        `;
        const statsResult = await env.BETTERSANTACRUZ_DB.prepare(statsSql)
          .bind(termId)
        ...
        const docStatsSql = `
          ...
          GROUP BY type
        `;
        const docStatsResult = await env.BETTERSANTACRUZ_DB.prepare(docStatsSql)
          .bind(termId)
